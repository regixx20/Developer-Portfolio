    ...
    <div id="projets" className="projects">
      ...
      <div className="projects__grid">
        {projects.map((project) => {
          ...
          const content = (
            <>
              <div className="projects__card-header">
                <h3>{project.name}</h3>
                {isUpdating && <span className="projects__status">{project.statusLabel ?? "Mise à jour à venir"}</span>}
              </div>
              {project.image ? (
                ...
                <div className="projects__thumbnail projects__thumbnail--placeholder" aria-hidden="true">
                  Aperçu en cours
                </div>
              )}
          ...
          return isUpdating ? (
            <article key={project.id} className={cardClassName} aria-disabled="true" aria-label={cardAriaLabel}>
              {content}
            </article>
          ) : (
            <Link
